...
const LocationInfoBox = ({ title, lat, lng, onClose }) => {
    ...
    const safeHavenApiCall = async () => {
        const shelters = await axios.get(`https://api.geoapify.com/v2/places?categories=service.social_facility.shelter,building.place_of_worship,education.library,education.school&filter=circle:${lng},${lat},32000&apiKey=${REACT_APP_GEOAPIFY_API_KEY}`);
        const shelterArr = []
    ...
    const hotelApiCall = async () => {
        const hotels = await axios.get(`https://api.geoapify.com/v2/places?categories=accommodation.hotel,accommodation.motel&filter=circle:${lng},${lat},32000&apiKey=${REACT_APP_GEOAPIFY_API_KEY}`);
        const hotelArr = []
    ...
    const hospitalApiCall = async () => {
        const hospitals = await axios.get(`https://api.geoapify.com/v2/places?categories=healthcare.hospital&filter=circle:${lng},${lat},32000&apiKey=${REACT_APP_GEOAPIFY_API_KEY}`);
        const hospitalArr = []
